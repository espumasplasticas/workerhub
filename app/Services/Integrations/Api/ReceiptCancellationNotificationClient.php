...
class ReceiptCancellationNotificationClient
{
    public function notifyReceiptCancelled(array $task): void
    {
        ...
        $endpoint = (string) config('services.api.receipt_cancellation_notifications_endpoint', '');
        ...
        if ($response->failed()) {
            throw new RuntimeException(sprintf(
                'API receipt cancellation notification failed with status %s.',
                $response->status()
            ));
        }
    }
