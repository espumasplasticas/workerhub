...
class InvoiceMigrationNotificationClient
{
    public function notifyInvoiceMigrated(array $task): void
    {
        ...
        $endpoint = (string) config('services.api.invoice_migration_notifications_endpoint', '');
        ...
        $response = Http::timeout((int) config('services.api.timeout', 10))
            ...
            ->post($baseUrl . '/' . ltrim($endpoint, '/'), [
                ...
                'invoice_total' => Arr::get($payload, 'invoice_total'),
        ...
        if ($response->failed()) {
            throw new RuntimeException(sprintf(
                'API invoice migration notification failed with status %s.',
                $response->status()
            ));
        }
    }
